...
import styles from './AndromedaBackdrop.module.css'

/**
 * Solary's Andromeda galaxy + starfield, drawn on a 2D canvas and tinted from
 * plain colour inputs. Ported from the Three.js scene's makeAndromedaTexture so
 * it can sit behind the device-preview iframe (and the marketing explorer)
 * without booting WebGL. Purely decorative (aria-hidden, pointer-events:none).
 */
...
export function AndromedaBackdrop({
  accent,
  sun,
  fg,
  dark,
  ...
  accent: string
  /** Secondary "sun"/highlight colour (palette.sun / swatches[1]). */
  sun: string
  fg: string
  dark: boolean
}) {
    ...
    <div className={styles.root} aria-hidden="true">
      ...
      {TWINKLES.map((s, i) => (
        <span
          ...
          style={{ left: `${s.x}%`, top: `${s.y}%`, width: s.r, height: s.r, background: fg, animationDelay: `${s.d}s` }}
        />
      ))}
    </div>
  )
...
export default AndromedaBackdrop
