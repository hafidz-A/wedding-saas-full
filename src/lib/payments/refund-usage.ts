...
export async function buildUsageSnapshot(
  db: any,
  invitationId: string,
  inv: { is_published?: boolean; paid_at?: string | null; updated_at?: string | null; used_at?: string | null; published_at?: string | null },
  ...
  return {
    ...
    // Sticky markers (DB triggers) — the robust, un-gameable signals.
    ever_used: !!inv.used_at,
    days_since_published: inv.published_at
      ? Math.max(0, Math.floor((nowMs - Date.parse(inv.published_at)) / 86_400_000))
      : null,
  }
}
