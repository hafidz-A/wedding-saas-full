...
export interface UsageSnapshot {
  ...
  ever_used: boolean
  /** STICKY: days since the invitation was FIRST published (null = never
   *  published). Measured from published_at, not the reversible is_published, so
   *  publish → share → unpublish can't dodge the grace window. */
...
export function refundVerdict(s: UsageSnapshot): RefundVerdict {
  ...
  // Sticky: measured from the FIRST publish (published_at), so unpublishing after
  // sharing the link can't dodge the grace window.
  if (s.days_since_published != null && s.days_since_published > REFUND_LIVE_GRACE_DAYS) return { eligible: false, code: 'used-live', label: `Tidak layak — sudah tayang > ${REFUND_LIVE_GRACE_DAYS} hari (§3)` }
