  ...
  return (
    <section style={card}>
      ...
      <div style={{ marginTop: 10 }}>
        <Button size="sm" variant="ghost" disabled={busy} onClick={download}>Unduh data saya (.json)</Button>
      ...
      {pending ? (
        <div>
          ...
          <Button size="sm" variant="ghost" disabled={busy} onClick={cancel} style={{ marginTop: 10 }}>Batalkan permintaan hapus</Button>
        </div>
      ) : confirming ? (
        <div style={{ border: '1px solid var(--status-error)', borderRadius: 'var(--radius-sm)', padding: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <Button size="sm" variant="danger" disabled={busy} onClick={submitDeletion}>Ya, ajukan hapus</Button>
            <Button size="sm" variant="ghost" disabled={busy} onClick={() => setConfirming(false)}>Batal</Button>
          </div>
        </div>
      ) : (
        <Button size="sm" variant="ghostDanger" onClick={() => setConfirming(true)}>Ajukan hapus akun…</Button>
      )}
