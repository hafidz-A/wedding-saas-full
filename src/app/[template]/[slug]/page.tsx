...
function addDemoSecondGallery(config: any) {
  ...
  // demo thumbnail (`<key>-sm.webp`, generated by
  ...
  //
  // Matches the extension loosely on purpose: the demo set was .jpg until
  // 2026-08-15 and any stored config written before then still names one. A
  // strict `.webp` check would silently stop thumbnailing those — no 404, just
  // full-size images quietly served into 150px cards.
  const toThumb = (src: string) =>
    typeof src === 'string' && /\.(jpe?g|webp)$/i.test(src) && !/-sm\.(jpe?g|webp)$/i.test(src)
      ? src.replace(/\.(jpe?g|webp)$/i, '-sm.webp')
      : src
